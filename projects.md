---
layout: default
title: Research Projects
---

<div class="project-item">
  <div class="experience-header">
    <div>
      <h2>Benchmarking Prediction of Mutational Effects on Protein-Protein Interactions using MAVE data</h2>
      <p class="company">Amelie Stein Group</p>
    </div>
    <div class="experience-meta">
      <span class="duration">February 2025 - May 2025</span>
      <span class="location">Copenhagen, Denmark</span>
    </div>
  </div>
  <p class="project-description">This Master's thesis evaluated computational methods for predicting how mutations affect protein-protein interactions using large-scale experimental data.</p>
  
  <h3 class="subsection-title">Key Contributions</h3>
  <ul class="experience-details">
    <li>Compiled and processed MAVE (Multiplexed Assays of Variant Effects) datasets</li>
    <li>Benchmarked multiple prediction algorithms for accuracy and reliability</li>
    <li>Analyzed performance across different protein families and interaction types</li>
    <li>Provided recommendations for method selection based on use case</li>
  </ul>
  
  <p class="project-tech"><span class="skill-label">Technologies:</span> Python, R, protein structure analysis tools, statistical modeling</p>
  <a href="https://github.com/antunes-beatriz/Thesis" target="_blank" class="github-link">
    → View Code & Report on GitHub
  </a>
</div>

<div class="project-item">
  <div class="experience-header">
    <div>
      <h2>Poly(A) Tail Detection with Nanopore Data using Gaussian HMM</h2>
      <p class="company">Amelie Stein Group</p>
    </div>
    <div class="experience-meta">
      <span class="duration">November 2024 - January 2025</span>
      <span class="location">Copenhagen, Denmark</span>
    </div>
  </div>
  <p class="project-description">This project focused on improving the detection of polyadenylation tail regions in RNA sequencing data from Oxford Nanopore Technologies.</p>
  
  <h3 class="subsection-title">Key Contributions</h3>
  <ul class="experience-details">
    <li>Analyzed nanopore sequencing data to identify poly(A) tail features</li>
    <li>Optimized detection algorithms by training a Gaussian Hidden Markov Model with curated training data</li>
    <li>Created a custom scoring function to evaluate prediction quality against Dorado baseline</li>
    <li>Demonstrated improved accuracy in tail length estimation for downstream analysis</li>
  </ul>
  
  <p class="project-tech"><span class="skill-label">Technologies:</span> Python, hmmlearn, nanopore sequencing data analysis</p>
  <a href="https://github.com/antunes-beatriz/PolyATail_project" target="_blank" class="github-link">
    → View Code & Report on GitHub
  </a>
</div>

<div class="project-item">
  <div class="experience-header">
    <div>
      <h2>Age of Runs of Homozygosity</h2>
      <p class="company">Amelie Stein Group</p>
    </div>
    <div class="experience-meta">
      <span class="duration">June 2024 - August 2024</span>
      <span class="location">Copenhagen, Denmark</span>
    </div>
  </div>
  <p class="project-description">This population genetics project investigated the temporal dynamics of genomic regions with reduced diversity.</p>
  
  <h3 class="subsection-title">Key Contributions</h3>
  <ul class="experience-details">
    <li>Assessed the correlation between ROH (Runs of Homozygosity) size and age through computational simulations</li>
    <li>Analyzed how demographic factors influence the distribution of ROH ages across populations</li>
    <li>Employed coalescent-based simulations to generate ancestral trees</li>
    <li>Identified ROH in sampled individuals and estimated their size and age distributions</li>
    <li>Provided insights into population history and inbreeding patterns</li>
  </ul>
  
  <p class="project-tech"><span class="skill-label">Technologies:</span> Python, msprime</p>
  <a href="https://github.com/antunes-beatriz/ROH-project" target="_blank" class="github-link">
    → View Code & Report on GitHub
  </a>
</div>
